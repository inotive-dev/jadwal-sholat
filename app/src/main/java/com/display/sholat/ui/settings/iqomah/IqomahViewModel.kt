 ...
 **/
class IqomahViewModel @Inject constructor(
    private val repository: RunningTextRepository,
    ...
    private val _background = MutableLiveData<Unit>()

    val responseAll = Transformations.switchMap(_all) { repository.getAllWithIqomah() }
    val responseCurrent = Transformations.switchMap(_current) { repository.getCurrent() }
    val responseBackground = Transformations.switchMap(_background) { slideShowRepository.getWithIqomah() }
    ...
    fun getBackground() = viewModelScope.launch {
        _background.value = Unit
    }

    fun updateBackground(slideShow: SlideShow) = viewModelScope.launch {
